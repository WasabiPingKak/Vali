...
最終匯入檔:`初學者的世界-mapmaking.json` = 40,000 自動配額點 + 68 手選點 = **40,068 點**,絕大多數鎖定官方 panoId。

手選檔:`locations/` 下所有 **GeoGuessr dict 格式**(`{name, customCoordinates}`)的檔案都會被分配腳本視為手選來源自動合併;裸陣列格式則是各國點池。目前有兩個:

- `beginner_world_handpick.json`(38 點):全球知名地標(雪梨歌劇院、自由女神、泰姬瑪哈、聖家堂等)+ EG 吉薩、CX 聖誕島、BR 巴西利亞
- `beginner_world_bm.json`(30 點):百慕達漢米爾頓(Vali 資料庫整島只有 113 點,程式產不出結果,改手選)
